...
The svgcheck project uses the **Git Feature Workflow** model.
...
As a general rule, work is never done directly on the svgcheck repository. You instead [create a fork](https://docs.github.com/en/get-started/quickstart/fork-a-repo) of the project. Creating a "fork" is producing a personal copy of the svgcheck project. Forks act as a sort of bridge between the original repository and your personal copy.

1. Navigate to https://github.com/ietf-tools/svgcheck
...
Right now, you have a fork of the svgcheck repository, but you don't have the files in that repository locally on your computer.

After forking the svgcheck repository, you should have landed on your personal forked copy. If that's not the case, make sure you are on the fork (e.g. `john-doe/svgcheck` and not the original repository `ietf-tools/svgcheck`).
...
```sh
git clone https://github.com/YOUR-USERNAME/svgcheck
```
4. Press **Enter**. Your local clone will be created in a subfolder named `svgcheck`.
...
2. In a terminal window, navigate to where you want to work. Subfolders will be created for each project you clone. **DO NOT** create empty folders for projects to be cloned. This is done automatically by git.
3. Type `gh repo clone` followed by `YOUR-USERNAME/svgcheck` (replacing YOUR-USERNAME with your GitHub username), e.g.:
```sh
gh repo clone john-doe/svgcheck
```
4. Press **Enter**. Your local clone will be created in a subfolder named `svgcheck`.
...
Make sure the base repository is set to `ietf-tools/svgcheck` with the branch `main` (this is the destination):
...
* Please shoot for a test suite with at least 80% code coverage for new code, as measured by the built-in coverage tests for the svgcheck or standalone use of coverage.py for other Python projects. For non-Python projects, use the most appropriate test coverage measurement tool.
* For the svgcheck, aim for 100% test suite template coverage for new templates.
